...
class Connection {
  ...
  public function get_issue($id) {
     ...
     * @return Issue
     */
    public function create_issue($project, $summary, $params = array()) {
  ...
  public function get_issues_for_project($project_id, $filter, $with, $after = NULL, $max = NULL) {
    $params = array(
    ...
    foreach ($with as $field) {
      $query .= "&with=" . urlencode($field);
    }

    $this->_clean_url_parameters($params);
    $xml = $this->_get('/project/issue/'. urldecode($project_id) .'?'. $query);
    $issues = array();
  ...
  public function get_issues($filter, $with, $max = NULL, $after = NULL) {
    $params = array(
      'filter' => (string)$filter,
    );
